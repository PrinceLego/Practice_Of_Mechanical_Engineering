...
\begin{itemize}
    \item 1100826 王子晨：循線演算法設計、書面報告撰寫與整理、簡報製作
    \item 1100854 蘇威全：風扇設計、流力分析、車體製作
    \item 1100862 施廷翰：車體設計、車體分析、車體製作
...
演算法的輸入為影像，演算法的輸出為車輛為$V$與$\omega$，定義如下圖\ref{fig:10}所示。
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{abc.png}     %圖片檔案名稱
    \caption{辨識前方偏移量和偏移角度示意圖}    %圖片檔案名稱
    \label{fig:abc}    %為圖片添加標籤
    %如\ref{fig:example1}所示
\end{figure}
%==============================內文==============================
...
接著是定義控制規則庫，$V$之控制規則表與$\omega$控制規則表，如下所示。
...
最後即可取得不同偏差角度$angle$及位置偏移量$distance$對應之前進速度$V_{x}$(圖\ref{fig:25})與角速度$\omega$(圖\ref{fig:27})。
...
最後進行實地的模擬，模擬跑道上的三種情形，包括直線(圖\ref{fig:fuzzy_st})、入彎(圖\ref{fig:fuzzy_in})、出彎(圖\ref{fig:fuzzy_out})。
圖為車輛靜止時所提拍攝之影像與其影像處理結果和模糊控制之結果（結果顯示於終端機）。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{fuzzy_st.jpg}     %圖片檔案名稱
    \caption{車輛靜止時影像處理與模糊控制之結果(直線)}    %圖片檔案名稱
    \label{fig:fuzzy_st}    %為圖片添加標籤
    %(圖\ref{fig:fuzzy_st})
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{fuzzy_in.jpg}     %圖片檔案名稱
    \caption{車輛靜止時影像處理與模糊控制之結果(入彎)}    %圖片檔案名稱
    \label{fig:fuzzy_in}    %為圖片添加標籤
    %如\ref{fig:fuzzy_st}所示
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{fuzzy_out.jpg}     %圖片檔案名稱
    \caption{車輛靜止時影像處理與模糊控制之結果(出彎)}    %圖片檔案名稱
    \label{fig:fuzzy_out}    %為圖片添加標籤
    %如\ref{fig:fuzzy_st}所示
\end{figure}

由上圖可知，在這三種情形，都有擬合出偵測區域中的標直，並做出相對應的控制。因此判斷此方法的可行性。
%==============================內文==============================
...
\begin{figure}[H]
    ...
    \caption{車輛速度對應角速度時間圖}    %圖片檔案名稱
    \label{fig:0_0_02_4}    %為圖片添加標籤
    %如\ref{fig:2}所示
\end{figure}

圖\ref{fig:0_0_02_7}及圖\ref{fig:0_0_02_8}為車輛對沿途對標線的偏移量與角度差(以右、順時針為正，以正前方為0度)，由圖(圖\ref{fig:0_0_02_8})可知最大偏移量為6.1$cm$，，由圖(圖\ref{fig:0_0_02_7})可知最大的角度差為17度。
\begin{figure}[H]
    \centering
...
%==============================內文==============================
\hspace{2em}
該部分由施廷翰同學一人負責，在繳交期限前已多次催促，但始終未交付任何實作內容，亦無回應或說明。
我已盡力嘗試補足書面報告內容，但由於其未透露太多細節，且實作與當初設計有所出入，相關內容最終仍無法完成，只能遺憾略過，實屬無奈。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{LINE_capture_772034270.455148.jpg}     %圖片檔案名稱
    \caption{LINE對話截圖}    %圖片檔案名稱
    \label{fig:line}    %為圖片添加標籤
\end{figure}
%==============================內文==============================
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{30.jpg}     %圖片檔案名稱
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{31.jpg}     %圖片檔案名稱
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{32.jpg}     %圖片檔案名稱
...
\subsection{結論} 
%==============================內文==============================
\hspace{2em}完成控制演算法的設計與模擬、風扇測試、車輛設計與實作。
%==============================內文==============================

\subsection{反思} 
%==============================內文==============================
\hspace{2em}本組在合作過程中，實際上缺乏整合與有效溝通。施廷翰同學原表示車輛製作由他負責，但多次於進度報告及期末報告前失聯，完全無法聯絡，也未明確交代其實作內容，導致相關進度難以掌握與配合。

負責簡報製作的魏羽暘同學，也多次在報告前未完成簡報內容，簡報大多由我在報告當天臨時製作完成，以確保報告能如期進行。

原本書面報告的撰寫並非由我負責，但因盧昀序同學中途退出，魏羽暘同學所撰寫的內容亦完全未達教授要求，自期初報告後，我便接手負責書面報告的撰寫工作。

然而，蘇威全同學與施廷翰同學的相關內容皆未讓我參與，且提供資訊有限，導致我無法整合全組資料，只能盡力完善我所掌握的部分，也因此期末報告仍有未盡周全之處，實屬無奈。
(BY 王子晨)
%==============================內文==============================
...
\subsection{程式碼} 
%==============================內文==============================
\hspace{2em}影像偵測演算法以 Python 撰寫，模糊控制模擬Matlab使用撰寫並模擬;
撰寫報告則使用 LaTeX 系統進行編輯與排版；
